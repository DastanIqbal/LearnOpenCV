...
background = cv2.imread('weather.png', cv2.IMREAD_COLOR)
...
def backgroundSelector(action, x, y, flag, userdata):
    ...
    if action == cv2.EVENT_LBUTTONDOWN:
        ...
        opposite_corner = (x, y)
        image_patch = frameYCrCb[min(corner[1], opposite_corner[1]):max(corner[1], opposite_corner[1]),
                      min(corner[0], opposite_corner[0]):max(corner[0], opposite_corner[0]), 1:]
...
def distanceCalculator(frameYCrCb):
    ...
    dist = np.linalg.norm(
        cv2.merge(((frameYCrCb[:, :, 1] - background_color[0]), (frameYCrCb[:, :, 2] - background_color[1]))), axis=-1)
    return dist
    ...
    while video_capture.isOpened():
        ...
        if ret:
            ...
            if background_color is not None:
                ...
                alpha[below_tola] = 0
                alpha[above_tolb] = 1
                alpha[between_tola_tolb] = (alpha[between_tola_tolb] - (tola * max_dist / 100)) / (
                        (tolb * max_dist / 100) - (tola * max_dist / 100))
                ksize = 3 + 2 * softness
                ...
                if is_enabled:
                    ...
                    frame = cv2.add(255 * cv2.multiply(frame_float, alpha_merge, dtype=cv2.CV_64F),
                                    255 * cv2.multiply(1 - alpha_merge, background_float, dtype=cv2.CV_64F),
                                    dtype=cv2.CV_8U)
