...
img_path = 'blemish.png'
img_save_path = 'output.jpg'
...
def getWindowSize(img):
    ...
    return cv2.resize(img, tuple([0, 0]), fx=max(h_ratio, w_ratio), fy=max(h_ratio, w_ratio),
                      interpolation=interpolation).shape[:2]
...
# 1. Find Sobel X,Y
# 2. Find Mean of Sobel
# 3. calculate other reigion
# 4. Compare mean with other regions, and find best region/window
# 5. Apply Seamless Cloning with best region/window
def removeBlemish(x, y):
    ...
    print(f"Sobel List {neighbor_list}")
    ...
    if best_window != [row - r, row + r + 1, col - r, col + r + 1]:
        ...
        mask = mask * 255
        img_copy = cv2.seamlessClone(img_copy[best_window[0]:best_window[1], best_window[2]:best_window[3]], img_copy,
                                     mask, (x, y), cv2.NORMAL_CLONE)
        cv2.imshow('window', img_copy)
        print(f"best_gradient_mean {best_gradient_mean}")
        print(f"best_windownot {[row - r, row + r + 1, col - r, col + r + 1]}")
    else:
        print(f"best_gradient_mean {best_gradient_mean}")
        print(f"best_window {[row - r, row + r + 1, col - r, col + r + 1]}")
